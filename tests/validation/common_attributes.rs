...
fn test_null_id_value() {
    ...
    // Actually validate the resource
    let result = registry.validate_scim_resource(&null_id_user);
...
fn test_invalid_id_format_array() {
    ...
    match result {
        Err(ValidationError::InvalidIdFormat { id }) => {
            assert_eq!(id, r#"Array [String("123"), String("456")]"#);
        }
        Err(other) => panic!("Expected InvalidIdFormat error, got {:?}", other),
        Ok(_) => panic!("Expected validation to fail, but it passed"),
    }
...
fn test_invalid_id_format_object() {
    ...
    match result {
        Err(ValidationError::InvalidIdFormat { id }) => {
            assert_eq!(id, r#"Object {"nested": String("value")}"#);
        }
        Err(other) => panic!("Expected InvalidIdFormat error, got {:?}", other),
        Ok(_) => panic!("Expected validation to fail, but it passed"),
    }
...
/// Note: This test is deferred because it requires operation context
#[test]
#[ignore = "Deferred - requires operation context for create/update detection"]
fn test_client_provided_id_in_create() {
    // This test would require passing operation context to validation
    // to distinguish between create and update operations
    // For now, ID validation only checks format and presence
}
...
/// Note: Currently this test passes validation because meta.resourceType is optional in the current implementation
#[test]
#[ignore = "meta.resourceType is currently optional in validation"]
...
fn test_client_provided_meta_readonly_attributes() {
    // This test would require passing operation context to validation
    // to detect when clients provide readonly attributes like created, lastModified
    // For now, meta validation only checks structure and resourceType
}
