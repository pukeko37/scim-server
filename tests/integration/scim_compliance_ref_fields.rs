...
async fn test_group_members_include_ref_fields() {
    ...
    let created_user = server.provider()
    ...
    let created_group = server.provider()
        .create_versioned_resource("Group", group_data, &context)
        .await
        .expect("Failed to create group");

    // Step 3: Verify the created group has proper $ref field in members
    let group_json = server.serialize_resource_with_refs(created_group.resource(), context.tenant_id())
        .expect("Failed to serialize group with refs");
    let members = group_json["members"].as_array()
    ...
    assert_eq!(member["display"], "John Doe", "Member display should be set");
    ...
    let retrieved_group = server.provider()
        .get_versioned_resource("Group", created_group.resource().get_id().unwrap(), &context)
        .await
        .expect("Failed to retrieve group");

    let retrieved_group_json = server.serialize_resource_with_refs(retrieved_group.unwrap().resource(), context.tenant_id())
        .expect("Failed to serialize retrieved group with refs");
    let retrieved_members = retrieved_group_json["members"].as_array()
        .expect("Retrieved group should have members array");

    let retrieved_member = &retrieved_members[0];
    assert!(retrieved_member["$ref"].is_string(), "Retrieved member must have $ref field");
    assert_eq!(
...
async fn test_user_groups_include_ref_fields() {
    ...
    let user_json = server.serialize_resource_with_refs(created_user.resource(), context.tenant_id())
        .expect("Failed to serialize user with refs");

    assert!(user_json["groups"].is_array(), "User must have groups array");

    let groups = user_json["groups"].as_array()
    ...
    assert_eq!(group_ref["value"], group_id, "Group value should be group ID");
    assert!(group_ref["$ref"].is_string(), "Group reference must have $ref field");
    assert_eq!(
...
async fn test_bidirectional_membership_references() {
    ...
    let updated_user = server.provider()
        .update_resource("User", &user_id, user_update_data, &context)
        .await
        .expect("Failed to update user with group reference");

    // Step 4: Verify Group -> User reference has $ref
    let group_json = server.serialize_resource_with_refs(group.resource(), context.tenant_id())
        .expect("Failed to serialize group with refs");
    let group_members = group_json["members"].as_array().unwrap();
    assert!(group_members[0]["$ref"].is_string(), "Group member must have $ref");

    // Step 5: Verify User -> Group reference has $ref (client-maintained bidirectional)
    let updated_user_json = server.serialize_resource_with_refs(&updated_user, context.tenant_id())
        .expect("Failed to serialize updated user with refs");
    assert!(updated_user_json["groups"].is_array(), "User must have groups array");
    let user_groups = updated_user_json["groups"].as_array().unwrap();
    assert_eq!(user_groups.len(), 1, "User should be in exactly one group");
    assert!(user_groups[0]["$ref"].is_string(), "User group reference must have $ref");
    assert_eq!(
...
async fn test_update_operations_preserve_ref_fields() {
    ...
    let _group_json = server.serialize_resource_with_refs(group.resource(), context.tenant_id())
        .expect("Failed to serialize group with refs");
    ...
    let updated_group = server.provider()
        .conditional_update("Group", group_id, update_data, &group_version, &context)
        .await
        .expect("Failed to update group");

    let updated_group = match updated_group {
        scim_server::resource::version::ConditionalResult::Success(resource) => resource,
        _ => panic!("Expected Success result from conditional update"),
    };

    // Step 4: Verify $ref fields are preserved after update
    let updated_json = server.serialize_resource_with_refs(updated_group.resource(), context.tenant_id())
        .expect("Failed to serialize updated group with refs");
    let updated_members = updated_json["members"].as_array()
        .expect("Updated group should have members array");

    assert_eq!(updated_members.len(), 1, "Group should still have one member");
    let member = &updated_members[0];

    assert!(member["$ref"].is_string(), "Updated member must preserve $ref field");
    assert_eq!(
        member["$ref"].as_str().unwrap(),
        format!("https://example.com/v2/Users/{}", user_id),
        "Updated member $ref should still point to correct user URI"
    );
    assert_eq!(updated_json["displayName"], "Updated Group Name", "Display name should be updated");
}
...
async fn test_nested_group_membership_refs() {
    ...
    let parent_group = server.provider()
        .create_versioned_resource("Group", parent_group_data, &context)
        .await
    ...
    let child_group = server.provider()
        .create_versioned_resource("Group", child_group_data, &context)
        .await
        .expect("Failed to create child group");

    // Verify child group has proper $ref to parent group
    let child_group_json = server.serialize_resource_with_refs(child_group.resource(), context.tenant_id())
        .expect("Failed to serialize child group with refs");
    let child_members = child_group_json["members"].as_array()
    ...
    assert_eq!(group_member["value"], parent_id, "Member value should be parent group ID");
    assert_eq!(group_member["type"], "Group", "Member type should be Group");
    assert!(group_member["$ref"].is_string(), "Group member must have $ref field");
    assert_eq!(
...
async fn test_multiple_group_members_all_have_refs() {
    ...
    let members_data: Vec<Value> = user_ids.iter().enumerate().map(|(i, user_id)| {
        json!({
            "value": user_id,
            "$ref": format!("https://example.com/v2/Users/{}", user_id),
            "type": "User",
    ...
    let group_json = server.serialize_resource_with_refs(group.resource(), context.tenant_id())
        .expect("Failed to serialize group with refs");
    let group_members = group_json["members"].as_array()
        .expect("Group should have members array");

    assert_eq!(group_members.len(), 3, "Group should have 3 members");

    for (i, member) in group_members.iter().enumerate() {
        assert!(member["$ref"].is_string(), "Member {} must have $ref field", i);
        assert_eq!(member["type"], "User", "Member {} should be of type User", i);
        assert!(member["value"].is_string(), "Member {} must have value field", i);

        let expected_ref = format!("https://example.com/v2/Users/{}", member["value"].as_str().unwrap());
        assert_eq!(member["$ref"], expected_ref, "Member {} $ref should be correct", i);
    }
...
async fn test_ref_fields_use_correct_base_url() {
    ...
    assert!(member["$ref"].is_string(), "Server should automatically generate $ref field");
    ...
    assert!(ref_url.starts_with("https://scim.company.com"), "$ref should use configured domain");
    assert!(ref_url.contains("/v2/Users/"), "$ref should contain correct SCIM path");
...
async fn test_ref_fields_subdomain_multitenant() {
    ...
    assert!(member["$ref"].is_string(), "Server should generate $ref field for tenant");

    let ref_url = member["$ref"].as_str().unwrap();
    let expected_url = "https://acme.scim.company.com/v2/Users/".to_string() + &user_id;

    assert_eq!(ref_url, expected_url, "$ref should use subdomain tenant strategy");
    assert!(ref_url.starts_with("https://acme.scim.company.com"), "$ref should use tenant subdomain");
}
...
async fn test_ref_fields_path_based_multitenant() {
    ...
    assert!(member["$ref"].is_string(), "Server should generate $ref field for path-based tenant");

    let ref_url = member["$ref"].as_str().unwrap();
    let expected_url = format!("https://api.company.com/enterprise/v2/Users/{}", user_id);

    assert_eq!(ref_url, expected_url, "$ref should use path-based tenant strategy");
    assert!(ref_url.contains("/enterprise/v2/"), "$ref should include tenant in path");
}
...
async fn test_missing_tenant_error() {
    ...
    let group_schema = server.get_schema_by_id("urn:ietf:params:scim:schemas:core:2.0:Group")
        .expect("Group schema should exist").clone();
    let group_handler = create_group_resource_handler(group_schema);
    server.register_resource_type("Group", group_handler, vec![ScimOperation::Create])
        .expect("Failed to register Group resource type");
    ...
    let result = server.create_resource_with_refs("Group", group_data, &context).await;

    assert!(result.is_err(), "Should fail when tenant required but missing");
    let error_msg = result.unwrap_err().to_string();
    assert!(error_msg.contains("Tenant ID required"), "Error should mention missing tenant ID");
}
