...
fn test_invalid_data_type_string_as_number() {
    ...
    match result {
        ...
        Err(other) => panic!("Expected Custom error about userName type, got {:?}", other),
        Ok(_) => panic!("Expected validation to fail, but it passed"),
    }
}
...
/// Test Error #26: Invalid decimal value format
#[test]
...
fn test_invalid_datetime_format() {
    ...
    match result {
        Err(ValidationError::InvalidCreatedDateTime) => {
            // Expected error for invalid created datetime format
        }
...
fn test_datetime_format_validation() {
    ...
    let user_with_timezone = json!({
        ...
        "meta": {
            "resourceType": "User",
            "created": "2023-01-01T00:00:00-08:00",
            "lastModified": "2023-12-31T23:59:59.123Z"
        }
    });
...
fn test_multiple_data_type_errors() {
    ...
    match result {
        Err(ValidationError::InvalidAttributeType { attribute, .. }) => {
...
fn test_data_type_coercion_edge_cases() {
    ...
    // Actually validate the resource
    let result =
        registry.validate_json_resource_with_context("User", &edge_cases, OperationContext::Update);
...
mod coverage_tests {
    ...
    fn test_data_type_test_comprehensiveness() {
        ...
        // This is ensured by the test pattern requiring SchemaRegistry::new() and validate_json_resource("User", )
    }
}
